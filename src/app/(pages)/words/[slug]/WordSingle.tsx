...
function WordSingle({ slug }: { slug: string }) {
  ...
  return (
    <>
      ...
      {!error && (
        <>
          <div className="pb-2 border-b border-gray flex justify-between">
            {isFetching || status === 'pending' ? (
              <div className="mb-1">
                <Loader />
              </div>
            ) : (
              <span className="text-[#fff] text-2xl">{localWordData?.en}</span>
            )}
          ...
          <div className="mt-4">
            ...
            {isFetching || status === 'pending' ? (
              ...
              <ul className="pl-5 list-disc text-[#fff]">
